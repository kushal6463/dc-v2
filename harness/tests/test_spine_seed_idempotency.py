...
def test_build_models_returns_full_spine() -> None:
    ...
    assert counts == {
        ...
        "Platform": 16,  # 5 top-level + 11 sub-platforms (google/meta sub-channels)
    }
...
def test_dry_run_builds_without_db() -> None:
    ...
    assert len(results) == 32  # 1 Business + 9 Domain + 6 Product + 16 Platform
    ...
    assert "32 spine nodes" in buf.getvalue()
